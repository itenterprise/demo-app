...
public class SectionListAdapter extends BaseExpandableListAdapter {
	...
	public View getChildView(int groupPosition, int childPosition,
		...
		SectionItem oSectionItem = this.sections.get(groupPosition)
				.getSectionItems().get(childPosition);
        ...
        if (oSectionItem.getIcon() != null){
            ...
            int drawableResource = context.getResources().getIdentifier(oSectionItem.getIcon(),
                    "drawable", context.getPackageName());
            if (drawableResource != 0){
                Drawable icon = context.getResources().getDrawable(drawableResource);
